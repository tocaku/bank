...
const displayTransactions = function (account, transactions, sort = false) {
  containerTransactions.innerHTML = '';
  const formatTransactionsDate = function (date, locale) {
    const getDaystween2Dates = (date1, date2) =>
      Math.round(Math.abs((date2 - date1) / (1000 * 60 * 60 * 24)));

    const daysPassed = getDaystween2Dates(new Date(), date);
...
let currentAccount, currenLogOutTimer;
...
const startLogoutTimer = function () {
  ...
  const lo0utTumer = setInterval(logOutTimerCallback, 1000);
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    if (currenLogOutTimer) clearInterval(currenLogOutTimer);
    currenLogOutTimer = startLogoutTimer();
...
btnTransfer.addEventListener('click', function (e) {
  ...
  clearInterval(currenLogOutTimer);
  currenLogOutTimer = startLogoutTimer();
  if (
    transferAmount > 0 &&
